...
export async function generateAISchedule(
  ...
  console.log(`[AISchedule] ===== FILTER DEBUG =====`);
  ...
  // Filter: include ONLY overdue tasks, today's tasks, and tomorrow's tasks
  // Future tasks beyond tomorrow are NOT generated by AI scheduling
  ...
  const tasksForToday = pendingTasks.filter(t => {
    ...
    console.log(`[AISchedule] TASK: "${t.title}" | RAW: ${t.dueDate} | PARSED: ${taskDueDay.toDateString()} | STATUS: ${t.status} | OVERDUE: ${isOverdue} | TODAY: ${isToday} | TOMORROW: ${isTomorrow}`);
    
    // Include overdue tasks, today's tasks, and tomorrow's tasks ONLY
