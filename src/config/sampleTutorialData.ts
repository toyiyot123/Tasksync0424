...
/**
 * Builds a realistic sample AIScheduleResult shown on the AI Scheduler page
 * while the tutorial is active, so the background isn't cluttered with the
 * user's real (and possibly unschedulable) tasks.
 */
export function buildSampleAIScheduleResult(): any {
  const today = new Date();
  today.setHours(0, 0, 0, 0);

  const makeTask = (id: string, title: string, priority: 'low' | 'medium' | 'high', estimatedTime: number) => ({
    id,
    title,
    description: '',
    dueDate: today,
  ...
  const schedule = [
    {
      task: makeTask('sample-1', 'Complete Project Report', 'high', 90),
      startHour: 10,
      startTime: '10:00 AM',
      endTime: '11:30 AM',
      durationMins: 90,
      confidence: 92,
      reason: 'Scheduled during your peak productivity window for deep focus work.',
      scheduledDate: today,
    },
    {
      task: makeTask('sample-2', 'Team Meeting Prep', 'high', 45),
      startHour: 11,
      startTime: '11:45 AM',
      endTime: '12:30 PM',
      durationMins: 45,
      confidence: 85,
      reason: 'Placed right before lunch when collaborative tasks fit best.',
      scheduledDate: today,
    },
    {
      task: makeTask('sample-3', 'Email Clients', 'medium', 30),
      startHour: 14,
      startTime: '2:00 PM',
      endTime: '2:30 PM',
      durationMins: 30,
      confidence: 78,
      reason: 'Routine communication slotted into a moderate-energy window.',
      scheduledDate: today,
    },
    {
      task: makeTask('sample-4', 'Code Review', 'low', 60),
      startHour: 15,
      startTime: '3:00 PM',
      endTime: '4:00 PM',
      durationMins: 60,
      confidence: 71,
      reason: 'Lower-priority work scheduled during a flexible afternoon block.',
      scheduledDate: today,
    },
  ];
