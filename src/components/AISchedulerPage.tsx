...
import { Clock, TrendingUp, TrendingDown, Zap, AlertCircle, CheckCircle, Lock, Coffee, Inbox } from 'lucide-react';
...
const AISchedulerPage: React.FC<AISchedulerPageProps> = ({ notEnoughDataMessage, selectedTaskId, sampleData }) => {
  ...
  // In sample/tutorial mode, show the schedule as-is since the tasks are demo data not in the live store.
  const activeSchedule = isSampleMode
    ? schedule
    : schedule.filter(s => !completedIds.has(s.task.id) && liveTaskMap.has(s.task.id));
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-6 sm:py-8 space-y-8">
      ...
      {/* Insights */}
      {(insights.bestHours.length > 0 || insights.worstHours.length > 0) ? (
          ...
          <div className="space-y-4">
            {(() => {
              ...
              return activeSchedule.map((item, idx) => {
                ...
                const isTomorrow = item.scheduledDate.toDateString() === tomorrow.toDateString();
                // Tomorrow's tasks are a continuous flow from today — suppress the date header
                // so the user doesn't think the schedule resets on a new day.
                const suppressHeader = isTomorrow;
